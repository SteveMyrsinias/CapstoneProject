...
# PURPOSE: Displays the vehicle count in the top-left corner of the frame
...
def count_vehicles(idxs, boxes, classIDs, vehicle_count, previous_frame_detections, frame, LABELS, data):
	...
	if len(idxs) > 0:
		...
		for i in idxs.flatten():
			...
			if (LABELS[classIDs[i]] in data["yolo_video"]["list_of_vehicles"]):
				current_detections[(centerX, centerY)] = vehicle_count 
				if (not boxInPreviousFrames(previous_frame_detections, (centerX, centerY, w, h), current_detections, data["yolo_video"]["FRAMES_BEFORE_CURRENT"])):
					vehicle_count += 1
